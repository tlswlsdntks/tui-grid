...
If you set a numerical value to the `min` and `max` options, you can validate whether the cell data is within the designated range. If the cell data is _not_ in range, smaller than the `min` value or larger than the `max` value, the data is displayed in red.
...
You can validate whether the cell data adheres to the specified pattern by defining the regular expression in the `regExp` option. If the cell data _does not_ adhere to the designated pattern, the data is displayed in red.
...
You can validate the cell data with respect to the result of the function defined in the `validatorFn` option with the data as its parameter. If the result of the function is _not_ `truthy`, the data is displayed in red.
  ...
  columns: [
    {
      ...
      validation: {
        validatorFn: (value, row, columnName) =>
          Number(value) + Number(row['anotherColumn']) > 10000,
      },
    },
  ],
});
...
If you set `unique` option to `true`, you can validate whether the cell data is unique in the column. If the cell data _is not unique_ in the column, the data is displayed in red.
